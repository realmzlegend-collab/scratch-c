...
// 2. Serve Static Assets (This ensures logo.svg is accessible)
app.use(express.static(path.join(__dirname, '../frontend')));

// 3. Database Connection (Corrected for modern MongoDB drivers)
mongoose.connect(process.env.MONGODB_URI)
    .then(() => console.log('✅ Database Connected Perfectly'))
    .catch(err => console.error('❌ Database Connection Error:', err));

// 4. THE LOGO & ADS INJECTOR (Fixed to work without touching HTML files)
const injectAssets = (html) => {
    // Script to force the logo.svg onto any image tag or logo class
    const logoScript = `
    <script>
